...
# get function from antoher file
#lap from temp0 import get_temp
    ...
    for s in range(len(d)):
        for n in range(len(digit)):
            ...
            for m in range(len(digit[n])):
                a= a + str((digit[n][m])) + ","
                if(digit[n][m] == "on"):
                    pixels[m*2 + d[s][1]] = red
                    pixels[m*2 +1 + d[s][1]] = red
                pixels.show()
            a = a + ")"
            #print(a)
        time.sleep(wait)
...
def turn_all_digits_as_8():
    for s in range(len(d) ):
        numdig = d[s][1]
        for p in range(len(digit[7])): 
            pixels[p * 2 + numdig] = red 
            pixels[p * 2 + 1 + numdig] = red
            pixels.show()
        ...
        for m in range(7):
            if (n==0 and d0 ==0):
                ...
                if(digit[currTime[n]][m] == "on"):
                    pixels[m*2 + numdig] = red
                    pixels[m*2+1 + numdig] = red
                else:
                    pixels[m*2 + numdig] = black
                    pixels[m*2+1 + numdig] = black
            pixels.show()
...
#        if (counter%3 == 0):
#            # call function to get current temperature
#            if (diff > 30 * 60):
